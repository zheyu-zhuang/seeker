"""Region-of-interest geometry and crop/mask conversion utilities."""
...
def crop_with_box(
    ...
    # Jitter box (same box_px will be used for BOTH image and mask crops)
    if box_jitter > 0:
        ...
        scale = 1 + (torch.rand(B, device=box_px.device) * 2 - 1) * box_jitter
        w = w * scale
        h = h * scale  # keep aspect, or set h=w if you want square crops

        jitter_x = (torch.rand(B, device=box_px.device) * 2 - 1) * (box_jitter * w)
        jitter_y = (torch.rand(B, device=box_px.device) * 2 - 1) * (box_jitter * h)
        cx = cx + jitter_x
        cy = cy + jitter_y
...
def crop_and_resize(
    ...
    """
    Vectorised crop-and-resize using ROIAlign (bilinear, GPU-friendly).

    If `clamp=False` the boxes are clamped to image bounds first.
    If `False` we feed the raw boxes to ROIAlign – samples falling outside
    ...
    rois = torch.cat([batch_ix, boxes_clamped.float()], dim=1)  # (B,5)
    with torch.no_grad():
        crops = roi_align(
            imgs, rois, output_size=output_size, spatial_scale=1.0, aligned=True
        )
    return crops
