...
Checkpoints are distributed via `seeker setup-assets` (see the top-level
[README](../../README.md#4-set-up-assets)), not committed to git.

## `seeker_mimicgen_v1.0.pth`
...
`dinov3_vits16plus.pth` and excludes `vit.*` from its strict-compatibility
...
Downloaded by `seeker setup-assets` to `.weights/mimicgen.rvt2_heatmap.ckpt`
along with the other checkpoints — only needed if you're running
`method=rvt2`; `seeker`/`mirroraug`/`oracle` don't use it.

**License**: MIT, same as `seeker_mimicgen_v1.0.pth` above. Like that
checkpoint, the frozen DINOv3 backbone is stripped out (verified
byte-identical to `dinov3_vits16plus.pth` before stripping) via
...
`dinov3_vits16plus.pth` instead. Cuts the file from ~156MB to ~42MB.
...
| File | sha256 |
|---|---|
| `seeker_mimicgen_v1.0.pth` | `b0aa9d7272e8b93ddccc402959969eb52ae075e8595d0ddd478a1b39c5aacda1` |
| `dinov3_vits16plus.pth` | `4057cbaaad8c16657adb09d6815f28d4164eeba30532fde23f0d17313124caea` |
| `mimicgen.rvt2_heatmap.ckpt` | `996ea845bcbb1fc4b5a9e8c66671d83acafcbb89113f74009bf81bc94696212e` |
